...
<script runat="server">
    protected void StoreSol_RefershData(object sender, StoreReadDataEventArgs e)
    {
        ...
        try
        {
            ...
            store.DataSource = UyumSosyal.Moduls.Sosyal_Medya_Islemleri.Shared.SehirSariAlanListesi.GetFilterL(0, 1000);
            store.DataBind();
            //X.MessageBox.Hide();
        }
        catch(Exception ex)
        {
            Helper.Error("HedefKitleSehirListe'de web service hatası : " + ex.Message);
        }
    }

    protected void StoreSag_RefershData(object sender, StoreReadDataEventArgs e)
    {
        X.MessageBox.Show( new MessageBoxConfig() {
            Message = "Islem yapilirken lutfen bekleyiniz.",
            ProgressText = "Bekleyiniz...",
        ...
        {
            var store = (Store)sender;
            store.DataSource = UyumSosyal.Moduls.Sosyal_Medya_Islemleri.Shared.SehirSariAlanListesi.GetFilterR(0, 1000);
            store.DataBind();
            //X.MessageBox.Hide();
        }
        catch(Exception ex)
...
<ext:Panel runat="server" Width="700" Height="330">
    ...
    <Items>
        <ext:GridPanel
            ...
            MarginSpec="0 2 0 0">
            <Store>
                <ext:Store ID="StoreL" OnReadData="StoreSol_RefershData" runat="server">
                    <Listeners>
            ...
            <ColumnModel>
                <Columns>
                    <ext:Column runat="server" Text="Şehir" Width="160" DataIndex="desc" Flex="1" />
                </Columns>
            </ColumnModel>
            <View>
        ...
        <ext:GridPanel
            ...
            MarginSpec="0 0 0 3">
            <Store>
                <ext:Store ID="StoreR" OnReadData="StoreSag_RefershData" runat="server">
                    <Listeners>
                        <Load Handler="Ext.MessageBox.hide();" Delay="1000"/>
                    </Listeners>
                    <Model>
